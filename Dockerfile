...
ARG UDP2RAW_TAG_NAME=20200818.0
...
RUN apt update \
 && apt install -y --no-install-recommends wireguard-tools iptables nano net-tools wget tar apt-utils ca-certificates curl iperf3 socat qrencode procps openresolv\
 && apt clean \
